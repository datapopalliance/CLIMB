...
for entry in kobo_data:
    ...
    recruit_ids = []
    for i in range(1, 4):
        recruit_name = entry.get(f"ELIGIBLE/ELIGIBLE_YES/RECRUITMENT/RECRUITMENT1/RECRUIT{i}_NAME", "").strip()
        recruit_phone = clean_phone_number(entry.get(f"ELIGIBLE/ELIGIBLE_YES/RECRUITMENT/RECRUITMENT1/RECRUIT{i}_PHONE", "").strip())
