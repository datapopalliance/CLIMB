...
if TEST_ENVIRONMENT:
    ...
    FORM_UID = os.getenv("KOBO_API_TOKEN")  # (Adjust if needed)
...
def fetch_existing_airtable_records():
    # Now we key the dictionary by the cleaned phone number.
    existing_records = {}
    field_exists = False
    offset = None
    ...
    while True:
        ...
        if response.status_code == 200:
            ...
            for record in data.get("records", []):
                ...
                raw_phone = fields.get("Numéro de téléphone")
...
def insert_recruit(name, recruit_phone, parent_data):
    # Use parent's data to set referral fields.
    parent_phone = parent_data.get("phone", "")
    parent_carrier = parent_data.get("carrier", "")
    parent_id = parent_data.get("participant_id", "")
    
    payload = {"records": [{"fields": {
        "Prénom": name,
        "Numéro de téléphone": recruit_phone,
        "Date de soumission": datetime.now().strftime("%Y-%m-%d"),
        "Recruté par": str(parent_id),
        "ref_phone": parent_phone,
        "ref_carrier": parent_carrier
    }}]}
...
# ✅ Step 1: Fetch all existing Airtable records (keyed by phone number)
...
for entry in kobo_data:
    # Use the phone number to look up the existing record.
    phone = clean_phone_number(entry.get("Numéro de téléphone"))
    ...
    participant_data = existing_airtable_records.get(phone)
    if not participant_data:
        logger.warning(f"⚠️ No existing participant found for phone {phone}, skipping...")
        continue
    ...
    if last_processed_time and submission_time <= last_processed_time:
        logger.info(f"⚠️ Skipping phone {phone}, already processed.")
        continue

    # ✅ Update "Recruté par" for existing records based on phone number
    ...
    if update_response.status_code == 200:
        logger.info(f"✅ Updated 'Recruté par' for participant with phone {phone} to '{id_ref}'")
    else:
        logger.error(f"❌ Error updating 'Recruté par': {update_response.text}")
    
    logger.info(f"✅ Processing participant {id_participant} with phone {phone}...")
    ...
    for i in range(1, 4):
        ...
        if recruit_name and recruit_phone:
            # Look for an existing recruit by matching phone number exactly
            existing_recruit_id = None
            for rec in existing_airtable_records.values():
                if recruit_phone == rec["phone"]:
                    existing_recruit_id = rec["record_id"]
                    break
            if existing_recruit_id:
                recruit_ids.append(existing_recruit_id)
            else:
                new_recruit_id = insert_recruit(recruit_name, recruit_phone, participant_data)
                if new_recruit_id:
